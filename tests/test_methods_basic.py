...
from conformal_audit.methods.frequency_grouped import FrequencyGroupedCP
...
def test_frequency_grouped_cp_returns_one_set_per_sample():
    cal_probs = np.array([[0.9, 0.1], [0.2, 0.8], [0.6, 0.4], [0.3, 0.7]])
    cal_labels = np.array([0, 1, 0, 1])
    test_probs = np.array([[0.7, 0.3], [0.4, 0.6]])
    sets = FrequencyGroupedCP(alpha=0.2, n_groups=2).fit(cal_probs, cal_labels).predict_sets(test_probs)
    assert len(sets) == 2
    assert all(len(pred_set) >= 1 for pred_set in sets)
